...
declare module "axioledger" {
  // `app()` initiates a axioledger instance. Only `app()`'s `node:` property and
  ...
  // This lets you make a variant of `h()` which is aware of your axioledger
  ...
  // A axioledger instance typically has an initial state and a top-level view
  ...
  type App<S> =
    Readonly<AtLeastSomething<{
      ...
      // The mount node is where a axioledger instance will get placed.
      node: Node
    }>>
  ...
  type ElementVNode<S> = {
    ...
    // axioledger takes care of using native Web platform event handlers for us.
    events?:
    Record<
      string,
